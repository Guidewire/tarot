...
class PurgingTrackerTest extends FunSuite
                  with ShouldMatchers {

  test("Basic timed purging works as expected") {
    val m1 = PurgingTracker.withTimingWindow[Int](50.milliseconds)
    val m2 = m1 + 1
  ...
  test("Max size purging should throw exception if given negative size") {
    intercept[IllegalArgumentException] {
      PurgingTracker.withMaximumSize(-1)
    }
  }

  test("Basic max size purging works as expected") {
    val p1 = PurgingTracker.withMaximumSize[Int](0)
    (p1 + 0).toSeq should be ('empty)
    (p1 append 0 append 1).toSeq should be ('empty)

    val p2 = PurgingTracker.withMaximumSize[Int](1)
    (p2 + 0).toSeq should be (Seq(0))
    (p2 append 1 append 2).toSeq should be (Seq(2))

    val p3 = PurgingTracker.withMaximumSize[Int](3)
    ...
    val p4 = PurgingTracker.withMaximumSize[Int](10)
