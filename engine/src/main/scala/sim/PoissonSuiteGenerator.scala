  ...
  with    InjectableSuiteGenerator
{
  ...
  def apply(interval: Interval): Seq[(DateTime, UID[_])] = {
    ...
    val count = numAddSuites.getAndSet(0)
    val injected =
      for (_ <- 0 until count)
        yield (start, pickRandomKey(config.suiteKinds))
